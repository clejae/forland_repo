...
for per in per_lst:
    ...
    for year in range(min,max):
        ...
        if os.path.exists(shp_pth):
            ...
            target_ds_pth = r'raster\Inv_CropTypesLeCe_{0}_{1}_5m.tif'.format(state, year)
            ...
            gdal.RasterizeLayer(target_ds, [1], lyr, options=["ATTRIBUTE=ID_HaBl"]) #burn_values=[1])#burn_values=[1])#
