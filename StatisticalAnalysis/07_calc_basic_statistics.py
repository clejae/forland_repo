...
#         print(year,tile)
#         ras = gdal.Open(r'data\raster\grid_15km\{0}\CropTypes_{1}.tif'.format(tile,year))
#         arr = ras.ReadAsArray()
#         arr[ :, :][arr[ :, :] == 14] = 12
#
...
#     for t, tile in enumerate(tiles_lst):
#         print(tile)
#         ras = gdal.Open(r'data\raster\grid_15km\{0}\{1}_CropSeqType_v2_clean.tif'.format(tile, per))
#         arr = ras.ReadAsArray()
#
#         ndval = ras.GetRasterBand(1).GetNoDataValue()
#
...
#
#     with pd.ExcelWriter(r'data\tables\CropRotations\{0}_AreaOfCropSequenceTypes_v2_clean.xlsx'.format(per)) as writer:
...
def statistics_task(lock, tile):
    ...
    ## open tile crop sequence and crop sequence type
    ras_cst = gdal.Open(r'data\raster\grid_15km\{0}\{1}_CropSeqType_v2_clean.tif'.format(tile, per))
    arr_cst_bu = ras_cst.ReadAsArray()  # [y1:y2,x1:x2]
...
for per in ['2005-2011','2012-2018']:
    ...
    df.to_excel(r'data\tables\CropRotations\{0}_CropTypesInCST_v2_clean.xlsx'.format(per))
    ...
    # df_prop_seq.sum(0)
    with pd.ExcelWriter(r'data\tables\CropRotations\{0}_ShareOfCropTypesInCropSequences_v2_clean.xlsx'.format(per)) as writer:
        df_prop_area.to_excel(writer, sheet_name='PropOfAreaInCST', index=True)
        df_prop_seq.to_excel(writer, sheet_name='PropOfOccInCST', index=True)
