...
def workFunc(bl):
    ...
    ## Susannes Study Area
    # strata = [[0, 1], [1, 2]] # Cattle
    # out_name = '{0}_{1}_CSTArea-StudyAreaSusanne'.format(bl, per)

    ## Cattle
    strata = [[0, 1], [1,50000]] #[1, 100], [100, 20000]] # Cattle
    out_name = '{0}_{1}_CSTArea-PigNumbers_binary'.format(bl, per)
   ...
   # ------------------------------------------ LOAD DATA & PROCESSING ------------------------------------------#
    ...
    for cl in strata:
        ...
        # def workFunc(tile):
            ...
            # msk_pth = r'data\raster\grid_15km\{0}\{1}_StudyAreaSusanne_2011.tif'.format(tile,bl)
    ...
    with pd.ExcelWriter(r'data\tables\crop_sequence_types\{0}\{1}.xlsx'.format(bl,out_name)) as writer:
...
if __name__ == '__main__':
    joblib.Parallel(n_jobs=20)(joblib.delayed(workFunc)(bl) for bl in bl_lst)
