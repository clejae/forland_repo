...
for year in range(2005, 2007):#[2005, 2020]: #
    # for ind in ind_lst:
    def workFunc(ind):
        ...
        in_shp_pth = r"data\vector\InvClassified\BY\slices\{0}\InVekos_BY_{0}_{1}.shp".format(year, ind)
        out_folder = r"data\vector\InvClassified\BY\slices\{0}\InVekos_BY_{0}_{1}_temp\\".format(year, ind)
        ...
        print("\nRemove duplicates from Invekos polygons and check and correct for validity of the polygons", ind)
        forland_wrapper.removeDuplicates(in_shp_pth, no_dups_pth)
        forland_wrapper.validityChecking(no_dups_pth)

        # print(year, "\nIdentify intersections of no duplicates polygons and check and correct for validity of the intersections", ind)
        ...
        print(year, ind, "done! Duration:", duration , "min.")
