...
def workFunc(tile):
    ...
    for i, year in enumerate(range(min, max+1)):
        pth = r'data\raster\grid_15km\{}\{}_Oeko_{}.tif'.format(tile, bl, year)
        ras = gdal.Open(pth)
        arr = ras.ReadAsArray()
        arr[arr == 255] = 0
        fill_arr[i, :, :] = arr
    # msk_arr = np.prod(fill_arr, 0)
    msk_arr2 = np.sum(fill_arr, 0)

    gt = ras.GetGeoTransform()
    pr = ras.GetProjection()

    # out_pth = r'data\raster\grid_15km\{}\{}_OekoMask_{}-{}.tif'.format(tile, bl, min, max)
    out_pth2 = r'data\raster\grid_15km\{}\{}_OekoMaskSum_{}-{}.tif'.format(tile, bl, min, max)

    # raster.writeArrayToRaster(msk_arr, out_pth, gt, pr, 255, type_code = gdal.GDT_Byte)
    raster.writeArrayToRaster(msk_arr2, out_pth2, gt, pr, 255, type_code=gdal.GDT_Byte)
    print(tile, "done")
