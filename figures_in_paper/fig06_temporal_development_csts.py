...
out_pth = r"figures\plots\cst_areas\SA+BB+BV_CSTAreas-per_period.png"
...
name_lst = ['Saxony-Anhalt', 'Brandenburg', 'Bavaria']
...
ncol = 1
nrow = 3
fig, axs = plt.subplots(nrows=nrow, ncols=ncol, sharey=True, sharex=True, figsize=cm2inch(16, 19))
...
for b,bl in enumerate(bl_lst):
    ...
    ## prepare data frame
    df_pth = r"data\tables\CropRotations\{0}\{0}_CSTArea.xlsx".format(bl)
    ...
    axs[ix].set_xticklabels(9 * ['05-11','08-14','12-18'],fontdict={'size': 10})

    ## label y axis
    axs[ix].set_xlabel('Structural types per period', fontdict={'size': 10})
...
axs[0,].set_yticks(np.arange(0, 36, step=5))
axs[1,].set_yticks(np.arange(0, 36, step=5))
