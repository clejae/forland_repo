...
out_pth = r"figures\in_paper\EJA\Fig5_EJA2.png"
...
ncol = 2
nrow = 2
fig, axs = plt.subplots(nrows=nrow, ncols=ncol, sharey=True, figsize=cm2inch(17.6, 16))
...
for s in range(4):
    ...
    x = .5
    for mt in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I']:
        axs[ix].annotate(mt, xy=(x, -5), fontsize=10) # 35.5 -
        x += 3
    ...
    for i in range(0,len(xs),2):
        x = xs[i]
        label = 'BB'
        axs[ix].annotate(label, xy=(x - .4, -2), fontsize=10, rotation=90) # 35.5 -

    for j in range(1,len(xs),2):
        x = xs[j]
        label = 'LS'
        axs[ix].annotate(label, xy=(x - .4, -2), fontsize=10, rotation=90)

## Label x-axis in lower axis
axs[(1,0)].set_xlabel('Structural diversity', fontdict={'size': 10})
axs[(1,1)].set_xlabel('Structural diversity', fontdict={'size': 10})
