...
out_pth = r"figures\poster\Landscape21-temporal_dev_BB+BV.png"
...
ncol = 1
nrow = 2
fig, axs = plt.subplots(nrows=nrow, ncols=ncol, sharey=True, sharex=True, figsize=cm2inch(11.5, 8))
...
for b, bl in enumerate(bl_lst):
    ...
    axs[ix].set_yticks(np.arange(0, 31, step=10))
    axs[ix].set_yticklabels(range(0, 31, 10), fontdict={'size': 9})
    x_ticks = list(np.arange(0,36,1))
    del x_ticks[3::4]
    axs[ix].set_xticks(x_ticks)
    axs[ix].set_xticklabels(9 * ["05-11","08-14","12-18"], fontdict={'size': 9})

    ## label y axis
    axs[ix].set_xlabel('Structural diversity per period', fontdict={'size': 10})
    axs[ix].set_ylabel('Share of \ncropland [%]',fontdict={'size': 10})
    ...
    # axs[ix].set_title(name_lst[b], loc='left',fontdict={'size':11}) #, 'weight':'semibold'
...
axs[0,].set_yticks(np.arange(0, 31, step=10))
axs[1,].set_yticks(np.arange(0, 31, step=10))
