...
# for year in range(2005, 2020):
    print("\nStarting year", year)
    ref_pth = r'Clemens\data\vector\IACS\LS\slices\{0}\IACS_LS_{0}_1_temp\05_nodups_cleaned_02.shp'.format(year)
    out_pth = r'Clemens\data\vector\IACS\LS\IACS_LS_{0}_cleaned.shp'.format(year)
    ...
    for index in range(1,14):
        print(year, index)
        sl_pth = r'Clemens\data\vector\IACS\LS\slices\{0}\IACS_LS_{0}_{1}_temp\05_nodups_cleaned_02.shp'.format(
            year, index)
...
if __name__ == '__main__':
    joblib.Parallel(n_jobs=4)(joblib.delayed(workFunc)(year) for year in range(2011, 2020))
